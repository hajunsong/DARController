...
int ecatmaster::f_init_slave_initialize(int v_local_slave_count, int v_local_domain_count)
{
    ...
    /* 1. slave setup start   */
    v_area_ret = v_global_dec_interface->f_oper_slave_setup_init(v_local_slave_count, v_local_domain_count);
    if (v_area_ret < 0 )
    {
                                     ...
                                     bool v_local_servo_flag)
{
    ...
    /* added */
    f_oper_sdo_write(v_local_slave_number, 0x60FE, 2, 255, _32BIT);
...
void ecatmaster::f_oper_set_servo_state(int v_local_slv_inx)
{
    ...
    {
        v_global_ecat_slave[v_local_slv_inx].v_global_cur_err_state_num = 0;
        ...
        case step_error:	// error
            ...
            qDebug() << "ERROR" << v_local_slv_inx << v_global_ecat_slave[v_local_slv_inx].v_global_cur_status_word;
            v_global_ecat_slave[v_local_slv_inx].v_global_cur_err_state_num = f_oper_sdo_read(v_local_slv_inx, 0x603F, 0, _16BIT);
            error_code_display(v_global_ecat_slave[v_local_slv_inx].v_global_cur_err_state_num);
            qDebug() << "ERROR code" << f_oper_sdo_read(v_local_slv_inx, 0x603F, 0, _16BIT);
...
void ecatmaster::error_code_display(uint32_t code){
    if(code == 8976){//2310
        qDebug() << "ERROR_CONTINUOUS_OVER_CURRENT";
    }else if(code == 12560){//3110
        qDebug() << "ERROR_MAINS_OVER_VOLTAGE";
    }else if(code == 12576){//3120
        qDebug() << "ERROR_MAINS_UNDER_VOLTAGE";
    }else if(code == 12816){//3210
        qDebug() << "ERROR_DC_LINK_OVER_VOLTAGE";
    }else if(code == 12832){//3220
        qDebug() << "ERROR_DC_LINK_UNDER_VOLTAGE";
    }else if(code == 29456){//7310
        qDebug() << "ERROR_SPEED";
    }else if(code == 65316){//FF24
        qDebug()<< "ERROR_GATE_DRV_VOLTAGE_OVER";
    }else if(code == 65317){//FF25
        qDebug()<< "ERROR_GATE_DRV_VOLTAGE_UNDER";
    }else if(code == 65441){//FFA1
        qDebug() << "ERROR_DRV_TEMPERATURE";
    }else{
//        qDebug() << "ERROR:" << code;
        printf("ERROR:%d(%x)\n", code, code);
    }
                                                ...
                                                bool v_local_is_servo_driver)
{
    ...
    if(v_local_vendorID == 0x00000ba9)
    {
        if(v_local_productCODE == 0x00000001)
        {
            // make pana slave information
                                 ...
                                 COMMUNICAT_SIZE_TYPE v_local_datatype)
{
    ...
    case _8BIT:
    {
        v_area_ret = v_global_dec_interface->f_oper_sdo_write_8(v_local_slave_number, v_local_index,v_local_subindex,(uint8_t)v_local_data);
        if (v_area_ret < 0)
        {
    ...
    case _16BIT:
    {
        v_area_ret = v_global_dec_interface->f_oper_sdo_write_16(v_local_slave_number, v_local_index,v_local_subindex,(uint16_t)v_local_data);
        if (v_area_ret < 0)
        {
    ...
    case _32BIT:
    {
        v_area_ret = v_global_dec_interface->f_oper_sdo_write_32(v_local_slave_number, v_local_index,v_local_subindex,(uint32_t)v_local_data);
        if (v_area_ret < 0)
        {
...
//sdo write
// if servo driver mode of operation index send date
uint32_t ecatmaster::f_oper_sdo_read(int v_local_slave_number,
                                 uint16_t v_local_index,
                                 uint16_t v_local_subindex,
                                 COMMUNICAT_SIZE_TYPE v_local_datatype)
{
    ...
    case _8BIT:
    {
        v_area_ret = v_global_dec_interface->f_oper_sdo_read_8(v_local_slave_number, v_local_index,v_local_subindex,(uint8_t*)&output_data);
        if (v_area_ret < 0)
        {
            d_print( "sdo_8 read failed (%x) \n",v_local_index);
        }
    }
        break;
    case _16BIT:
    {
        v_area_ret = v_global_dec_interface->f_oper_sdo_read_16(v_local_slave_number, v_local_index, v_local_subindex, (uint16_t*)&output_data);
        if (v_area_ret < 0)
        {
            d_print( "sdo_16 read failed (%x) \n",v_local_index);
        }
    }
        break;
    case _32BIT:
    {
        v_area_ret = v_global_dec_interface->f_oper_sdo_read_32(v_local_slave_number, v_local_index,v_local_subindex,(uint32_t*)&output_data);
        if (v_area_ret < 0)
        {
            d_print( "sdo_32 read failed (%x) \n",v_local_index);
        }
    }
        break;
    }
...
//                v_area_ecat_control->f_get_slave()[6].f_oper_DIO(2); // orange

            // 5-1 : green
            // 5-2 : red
            // 6-2 : orange

////                // SDO reading test : may cause delay
////                qDebug() << "A" << v_area_ecat_control->f_oper_sdo_read(0, 0x60FE, 2, _32BIT);  //f_oper_sdo_write(v_local_slave_number, 0x60FE, 2, 255, _32BIT);
////                qDebug() << "B" << v_area_ecat_control->f_oper_sdo_read(0, 0x6060, 0, _8BIT);  //f_oper_sdo_write(v_local_slave_number, 0x60FE, 2, 255, _32BIT);
...
//                act_pos[v_sub_count] = v_area_ecat_control->f_get_slave()[v_sub_count].v_global_cur_position;
            }

//            /* DO COMM parsing, IK, FK, etc */


//            if(test_mode == 0){ // delay
//                delay_cnt++;
//                if(delay_cnt >= 1000){
//                    test_mode = 1;
//                    delay_cnt = 0;
//                    qDebug() << act_pos[0] << act_pos[1] << act_pos[2] << act_pos[3] << act_pos[4] << act_pos[5] << act_pos[6];
//                    qDebug() <<  v_area_ecat_control->v_des_pos[0]<< v_area_ecat_control->v_des_pos[1]<< v_area_ecat_control->v_des_pos[2]<< v_area_ecat_control->v_des_pos[3]<< v_area_ecat_control->v_des_pos[4]<< v_area_ecat_control->v_des_pos[5]<< v_area_ecat_control->v_des_pos[6];
//                    qDebug() << "---------";
//                }
//            }else if(test_mode == 1){ // position control
//                v_area_ecat_control->v_des_pos[0] = v_area_ecat_control->v_des_pos[0] - 2.;
//                v_area_ecat_control->v_des_pos[1] = v_area_ecat_control->v_des_pos[1] + 2.;
//                v_area_ecat_control->v_des_pos[2] = v_area_ecat_control->v_des_pos[2] - 2.;
//                v_area_ecat_control->v_des_pos[3] = v_area_ecat_control->v_des_pos[3] + 2.;
//                v_area_ecat_control->v_des_pos[4] = v_area_ecat_control->v_des_pos[4] - 2.;
//                v_area_ecat_control->v_des_pos[5] = v_area_ecat_control->v_des_pos[5] + 2.;
//                v_area_ecat_control->v_des_pos[6] = v_area_ecat_control->v_des_pos[6] - 2.;
//                delay_cnt++;
//                if(delay_cnt >= 7000){
//                    qDebug() << act_pos[0] << act_pos[1] << act_pos[2] << act_pos[3] << act_pos[4] << act_pos[5] << act_pos[6];
//                    qDebug() <<  v_area_ecat_control->v_des_pos[0]<< v_area_ecat_control->v_des_pos[1]<< v_area_ecat_control->v_des_pos[2]<< v_area_ecat_control->v_des_pos[3]<< v_area_ecat_control->v_des_pos[4]<< v_area_ecat_control->v_des_pos[5]<< v_area_ecat_control->v_des_pos[6];
//                    qDebug() << "Moving to CST mode";
//                    test_mode = 2;
//                    delay_cnt = 0;
//                }
//            }else if(test_mode == 2){ // to torque control
//                for(int v_sub_count =0; v_sub_count < v_area_ecat_control->f_get_slave_count(); ++v_sub_count){
//                    v_area_ecat_control->f_get_slave()[v_sub_count].f_oper_to_cst();
//                }
//                qDebug() << "CST mode";
//                run_mode = 1;
//                test_mode = 3;
//            }else if(test_mode == 3){ // torque control
//                v_area_ecat_control->v_des_tor[0] = 300.;
//                v_area_ecat_control->v_des_tor[1] = -300.;
//                v_area_ecat_control->v_des_tor[2] = 300.;
//                v_area_ecat_control->v_des_tor[3] = -300.;
//                v_area_ecat_control->v_des_tor[4] = 300.;
//                v_area_ecat_control->v_des_tor[5] = -300.;
//                v_area_ecat_control->v_des_tor[6] = 300.;
//                delay_cnt++;
//                if(delay_cnt >= 7000){
//                    delay_cnt = 0;
//                    test_mode = 4;
//                }
//            }else if(test_mode == 4){ // to position control
//                for(int v_sub_count =0; v_sub_count < v_area_ecat_control->f_get_slave_count(); ++v_sub_count){
//                    v_area_ecat_control->f_get_slave()[v_sub_count].f_oper_to_csp();
//                    v_area_ecat_control->v_des_pos[v_sub_count] = act_pos[v_sub_count];
//                }
//                qDebug() << "CSP mode";
//                run_mode = 0;
//                test_mode = 1;
